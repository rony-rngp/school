                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="float-left">Edit Employee Leave</h3>
                        ...
                        <div class="card-body">
                            <form id="quickForms" action="{{ route('update.employee.leave',$employee_leave->id) }}" method="POST" enctype="multipart/form-data" >
                                @csrf
                                <div class="form-row">
                                    <div class="form-group col-md-4">
                                        ...
                                        <select name="employee_id" id="employee_id" class="form-control select2bs4">
                                            ...
                                            @foreach($employees as $employee)
                                                <option {{ $employee_leave->employee_id == $employee->id ? 'selected' : '' }} value="{{ $employee->id }}">{{ $employee->name }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group col-md-4">
                                        ...
                                        <input type="text" name="start_date" id="start_date" value="{{ $employee_leave->start_date }}" class="form-control" placeholder="Start Date" autocomplete="off">
                                        <span style="color:red">{{ $errors->has('start_date') ? $errors->first('start_date') : '' }}</span>
                                    </div>

                                    <div class="form-group col-md-4">
                                        <label for="end_date">End Date</label>
                                        <input type="text" name="end_date" id="end_date" value="{{ $employee_leave->end_date }}" class="form-control" placeholder="End Date" autocomplete="off">
                                    ...
                                    <div class="form-group col-md-4">
                                        ...
                                        <select name="leave_purposes_id" id="leave_purposes_id" class="form-control select2bs4">
                                            ...
                                            @foreach($leave_purpose as $purpose)
                                                <option {{ $employee_leave->leave_purposes_id == $purpose->id ? 'selected' : '' }} value="{{ $purpose->id }}">{{ $purpose->name }}</option>
                                            @endforeach
